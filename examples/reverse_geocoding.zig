...
pub fn main() !void {
    ...
    const lat = 52.5162767;
    const lng = 13.3777025;
    ...
    try stdout.print("Reverse Geocoding Query: Lat={d:.7}, Lng={d:.7}\nOptions: {any}\n\n", .{ lat, lng, opts });

    var parsed_response = client.geocodeReverse(lat, lng, opts) catch |err| {
        try stderr.print("Reverse geocoding request failed: {s}\n", .{@errorName(err)});
        switch (err) {
            ...
            opencage.Error.BadRequest => try stderr.print(" -> Bad Request (check coordinates or parameters).\n", .{}),
            ...
            opencage.Error.RequestTooLong => try stderr.print(" -> Request query was too long (unlikely for reverse).\n", .{}),
    ...
    if (response.results.len > 0) {
        ...
        for (response.results, 0..) |result, i| {
            ...
            try stdout.print("  State:     ", .{});
            try opencage.safeWriteUtf8Optional(stdout, result.components.state);
            try stdout.print("\n", .{});

            try stdout.print("  City:      ", .{});
            try opencage.safeWriteUtf8Optional(stdout, result.components.city);
            try stdout.print("\n", .{});
